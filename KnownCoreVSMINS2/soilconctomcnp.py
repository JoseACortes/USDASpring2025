...
def sample_section(f, midpoints, padding, n):
    ...
    return elems
# %%
def make_mcnp(f, extent, res, elem_labels, density = -2.156, x_fix = 0, y_fix = 0, z_fix=0, z_mul = 1, mw='w', surface_header='', surface_footer='', mat_header='', mat_footer='', cell_header='', cell_footer='', subsection_n=50, blur_dist = None):
    """
    ...
    if blur_dist is not None:
        elems[:, 0] = elems[:, 0]+blur_dist(1)
        elems[:, 0] = np.clip(elems[:, 0], 0, 1)
        elems[:, 1] = elems[:, 1]+blur_dist(1)
        elems[:, 1] = np.clip(elems[:, 1], 0, 1)
        elems[:, 2] = 1-elems[:, 0]-elems[:, 1]
    ...
    return cells, walls, surfaces, mats, avg_sample
